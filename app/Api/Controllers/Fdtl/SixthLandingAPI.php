...
class SixthLandingAPI {

    public static function insert_data($data) {
        $id = $data['id'];
        $current_landing = $data['current_landing'];
        $aircraft_callsign = $data['aircraft_callsign'];
        $departure_aerodrome6 = $data['departure_aerodrome6'];
        $destination_aerodrome6 = $data['destination_aerodrome6'];
        $date_of_flight6 = $data['date_of_flight6'];
        $entered_departure_time6 = $data['departure_time6'];
        $combined_total_time6 = $data['total_flying_time6'];
        $departure_time_date_format6 = strtotime($entered_departure_time6);
        $total_flying_time6 = strtotime($combined_total_time6);

        //Add Dep Time and Total Flying Time
        $secs = strtotime($combined_total_time6) - strtotime("0000");
        $dep_time_total_time6 = date('Hi', strtotime($entered_departure_time6) + $secs);
...
//        $reporting_time5 = $previous_reporting_time[4]['reporting_time'];
//        $flight_time5 = $previous_flight_time[4]['flight_time'];
//        $total_FT5 = $previous_total_ft[4]['total_ft'];
//        $duty_end_time5 = $previous_duty_end_time[4]['duty_end_time'];
...
//        $split_duty5 = $previous_split_duty[4]['split_duty'];
...
//        $dep_time_total_time5 = $previous_dep_time_total_time[4]['dep_time_total_time'];
...
//                4->first
         ...
         $reporting_time1 = $previous_reporting_time[4]['reporting_time'];
        $reporting_time5 = $previous_reporting_time[0]['reporting_time'];
        $flight_time5 = $previous_flight_time[0]['flight_time'];
        $total_FT5 = $previous_total_ft[0]['total_ft'];
        $duty_end_time5 = $previous_duty_end_time[0]['duty_end_time'];
        $split_duty2 = $previous_split_duty[3]['split_duty'];
        $split_duty3 = $previous_split_duty[2]['split_duty'];
        $split_duty4 = $previous_split_duty[1]['split_duty'];
        $split_duty5 = $previous_split_duty[0]['split_duty'];
        $dep_time_total_time1 = $previous_dep_time_total_time[4]['dep_time_total_time'];
        $dep_time_total_time2 = $previous_dep_time_total_time[3]['dep_time_total_time'];
        $dep_time_total_time3 = $previous_dep_time_total_time[2]['dep_time_total_time'];
        $dep_time_total_time4 = $previous_dep_time_total_time[1]['dep_time_total_time'];
        $dep_time_total_time5 = $previous_dep_time_total_time[0]['dep_time_total_time'];

        $previous_split_duty_values = $split_duty2 . '' . $split_duty3 . '' . $split_duty4 . '' . $split_duty5;


        $reporting_time6 = date('Hi', strtotime($reporting_static_time, $departure_time_date_format6));
        $flight_time6 = date('Hi', strtotime($flight_static_time, $total_flying_time6));
        $chocks_off6 = date('Hi', strtotime($chocks_off_static_time, $departure_time_date_format6));



        $chocks_on6 = date('Hi', strtotime($chocks_on_static_time, strtotime($dep_time_total_time6)));

        $duty_end_time6 = date('Hi', strtotime($duty_end_static_time, strtotime($chocks_on6)));


        //IST times
        $reporting_time_ist6 = "(" . date('Hi', strtotime('+5 hour +30 minutes', strtotime($reporting_time6))) . " IST)";
        $chocks_off_ist6 = "(" . date('Hi', strtotime('+5 hour +30 minutes', strtotime($chocks_off6))) . " IST)";
        $chocks_on_ist6 = "(" . date('Hi', strtotime('+5 hour +30 minutes', strtotime($chocks_on6))) . " IST)";
        $duty_end_time_ist6 = "(" . date('Hi', strtotime('+5 hour +30 minutes', strtotime($duty_end_time6))) . " IST)";

        //Display Hours and Minutes
        $flight_time_only_hours6 = substr($flight_time6, 0, 2);
        $flight_time_hours_format6 = ($flight_time_only_hours6 <= "01") ? " Hour " : " Hours ";
        $flight_time_only_Minutes6 = substr($flight_time6, 2, 2);
        $flight_time_minutes_format6 = ($flight_time_only_Minutes6 <= "01") ? " Minute " : " Minutes ";

        //Subtract $duty_end_time - $reporting_time
        $secs = strtotime($reporting_time6) - strtotime("0000");
        $flight_duty_time6 = date('Hi', strtotime($duty_end_time6) - $secs);

        $flight_duty_time_only_hours6 = substr($flight_duty_time6, 0, 2);
        $flight_duty_time_hours_format6 = ($flight_duty_time_only_hours6 <= "01") ? " Hour " : " Hours ";
        $flight_duty_time_only_Minutes6 = substr($flight_duty_time6, 2, 2);
        $flight_duty_time_minutes_format6 = ($flight_duty_time_only_Minutes6 <= "01") ? " Minute " : " Minutes ";

        //split duty calculations
        $last_DET = strtotime($duty_end_time5) - strtotime("0000");
        $split_duty = date('Hi', strtotime($reporting_time6) - $last_DET);
        ...
        $fifth_total_ft = strtotime($total_FT5) - strtotime("0000");
        $total_FT6 = date('Hi', strtotime($flight_time6) + $fifth_total_ft);

        $total_FT_only_hours = substr($total_FT6, 0, 2);
        $total_FT_hours_format = ($total_FT_only_hours <= "01") ? " Hour " : " Hours ";
        $total_FT_only_Minutes = substr($total_FT6, 2, 2);
        ...
        if ((($dep_time_total_time1 >= $night_time_from) && ($dep_time_total_time1 <= $night_time_to)) ||
                ...
                (($dep_time_total_time5 >= $night_time_from) && ($dep_time_total_time5 <= $night_time_to))) {
            // Night timings
        ...
        $total_fdp6 = date('Hi', strtotime($duty_end_time6) - $First_RT_value1);
        //IST format
        $total_fdp_ist = date('Hi', strtotime('+5 hour +30 minutes', strtotime($total_fdp6)));

        $total_fdp_only_hours = substr($total_fdp6, 0, 2);
        $total_fdp_hours_format = ($total_fdp_only_hours <= "01") ? " Hour " : " Hours ";
        $total_fdp_only_Minutes = substr($total_fdp6, 2, 2);
        ...
        $first_char_entered_departure_time = substr($entered_departure_time6, 0, 2);
        $last_char_entered_departure_time = substr($entered_departure_time6, -2, 2);
        $first_char_dep_time_total_time = substr($dep_time_total_time6, 0, 2);
        $last_char_dep_time_total_time = substr($dep_time_total_time6, -2, 2);

        if ((($entered_departure_time6 >= "2030") && ($entered_departure_time6 <= "2359")) ||
            ...
            $flight_duty_time_convert = strtotime($flight_duty_time6) - strtotime("0000");
            ...
            if (strpos($split_duty_display_format, "APPLICABLE") != false) {
                $last_arrival_time_value6 = $last_arrival_time;
                ...
                $last_arrival_time_value6 = date('Hi', strtotime($last_arrival_time) + $split_duty_value_strtotime);
                $last_arrival_time = date('H:i', strtotime($last_arrival_time) + $split_duty_value_strtotime) . " (WITH SPLIT DUTY)";
            }
        } else if ((($dep_time_total_time6 >= "2030") && ($dep_time_total_time6 <= "2359")) ||
            ...
            date_default_timezone_set('UTC');
            $flight_duty_time_format = strtotime($flight_duty_time6) - strtotime("0000");
        ...
        $total_fly_time = strtotime($combined_total_time6) - strtotime("0000");
        $last_dep_time = date('Hi', strtotime($last_arrival_time) - $total_fly_time);
        $last_dep_time_value6 = $last_dep_time;
        ...
        if ($total_fdp6 < $twelve_hours) {
            $fourteen_thirty = strtotime($fourteen_thirty) - strtotime("0000");
            $next_day_earliest_dep_time = date('H:i', strtotime($duty_end_time6) + $fourteen_thirty);
            $next_day_earliest_dep_time_value6 = date('H:i', strtotime($duty_end_time6) + $fourteen_thirty);
            ...
            $total_fdp = strtotime($total_fdp6) - strtotime("0000");
            $next_day_earliest_dep_time = date('H:i', strtotime($duty_end_time6) + $two_thirty + $total_fdp);
            $next_day_earliest_dep_time_value6 = date('Hi', strtotime($duty_end_time6) + $two_thirty + $total_fdp);
            $next_day_earliest_dep_time_ist = "(" . date('Hi', strtotime('+5 hour +30 minutes', strtotime($next_day_earliest_dep_time))) . " IST)";
        }


        $result = [
            "dep_time_total_time6" => $dep_time_total_time6,
            "reporting_time6" => $reporting_time6 . " GMT " . $reporting_time_ist6,
            "chocks_off6" => $chocks_off6 . " GMT " . $chocks_off_ist6,
            "chocks_on6" => $chocks_on6 . " GMT " . $chocks_on_ist6,
            "duty_end_time6" => $duty_end_time6 . " GMT " . $duty_end_time_ist6,
            "flight_time6" => $flight_time_only_hours6 . '' . $flight_time_hours_format6 . '' . $flight_time_only_Minutes6
            . '' . $flight_time_minutes_format6,
            "flight_duty_period6" => $flight_duty_time_only_hours6 . "" . $flight_duty_time_hours_format6
            . "" . $flight_duty_time_only_Minutes6 . "" . $flight_duty_time_minutes_format6,
            "split_duty6" => $split_duty_display_format,
            'split_duty6_condition_value' => $split_duty_condition_value,
            "total_FT6" => $total_FT_display_format,
            'total_FT_condition_value' => $total_FT_condition_value,
            "total_fdp6" => $total_fdp_display_format,
            'total_fdp_condition_value' => $total_fdp_condition_value,
            "last_dep_time6" => $last_dep_time,
            "last_arrival_time6" => $last_arrival_time,
            "next_day_take_off6" => $next_day_earliest_dep_time . " GMT " . $next_day_earliest_dep_time_ist,
            'reporting_time6_value' => $reporting_time6,
            'flight_time6_value' => $flight_time6,
            'chocks_off6_value' => $chocks_off6,
            'chocks_on6_value' => $chocks_on6,
            'duty_end_time6_value' => $duty_end_time6,
            'flight_duty_period6_value' => $flight_duty_time6,
            'split_duty6_value' => $split_duty_calculation_value,
            'total_FT6_value' => $total_FT6,
            'total_fdp6_value' => $total_fdp6,
            'last_dep_time6_value' => $last_dep_time_value6,
            'last_arrival_time6_value' => $last_arrival_time_value6,
            'next_day_take_off6_value' => $next_day_earliest_dep_time_value6,
        ];
        return $result;
    }
