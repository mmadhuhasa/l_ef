    ...
    var multipleDatePicker = function() {
        return {
            ...
            template: '<div class="multiple-date-picker">' +
                ...
                '<div class="text-center day-of-week"  ng-repeat="day in daysOfWeek"><input type="checkbox" class="weekday-checkbox" ng-model="weekDaysArr[$index]" ng-click="weekDayClick(day,weekDaysArr[$index])" ng-disabled="disableWeekCheck[day]"/>{{day}}</div>' +
                '</div>' +
            ...
            link: function(scope) {
                ...
                var checkNavigationButtons = function() {
                    ...
                    getDaysOfWeek = function() {
                        ...
                        var momentDaysOfWeek = moment().localeData()._weekdays,
                            days = [];
                            // console.log(momentDaysOfWeek);
                        for (var i = 1; i < 7; i++) {
                            days.push(momentDaysOfWeek[i].substring(0,3));
                        }

                        if (scope.sundayFirstDay) {
                            days.splice(0, 0, momentDaysOfWeek[0].substring(0,3));
                        } else {
                ...
                scope.$on('onDaily', function(event, data) {
                    scope.weekDaysArr = [];
                    // console.log(event, data);
                    // console.log(scope.weekDaysArr);


                });
                scope.$on('setWeekValues', function(event, data) {
                        // console.log('setWeekValues');
                        // console.log(data.data);
                        for (var key in data.data) {
                            var weekDaysName = data.data[key].split(',');
                        }
                        // console.log()
                        if (weekDaysName[0] == "" && weekDaysName.length == 1) {
                            console.log('reset');
                            var nameArr = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
                            // scope.weekDaysArr[false, false, false, false, false, false, false];
                            for (var i = 0; i < 7; i++) {
                                scope.weekDaysArr[i] = false;
                                scope.$emit('weekDayClick', {name:nameArr[i],val:false});
                            };
                            return;
                        }
                        for (var i = 0; i < weekDaysName.length; i++) {
                            switch (weekDaysName[i]) {
                                case 'sun':
                                    scope.weekDaysArr[0] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                                case 'mon':
                                    scope.weekDaysArr[1] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                                case 'tue':
                                    scope.weekDaysArr[2] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                                case 'wed':
                                    scope.weekDaysArr[3] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                                case 'thu':
                                    scope.weekDaysArr[4] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                                case 'fri':
                                    scope.weekDaysArr[5] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                                case 'sat':
                                    scope.weekDaysArr[6] = true;
                                    scope.$emit('weekDayClick', {name:weekDaysName[i].substring(0, 1).toUpperCase() + "" + weekDaysName[i].substring(1, 3),val:true});
                                    break;
                            }
                            // console.log(scope.weekDaysArr);
                    ...
                    /* Week day click */
                scope.weekDayClick = function(name,val) {
                    // console.log(scope.weekDaysArr);
                    // console.log(name);
                    scope.$emit('weekDayClick', {name:name,val:val});
                };
