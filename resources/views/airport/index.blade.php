...
<style type="text/css">
  ...
  .fav_airports_tabs .nav-tabs {border:none;}
  .fav_airports_tabs .nav-tabs>li>a {border-radius: 4px;line-height: 0.5;margin: 0 9px;}
  .fav_airports_tabs .nav-tabs>li.active>a,.fav_airports_tabs .nav-tabs>li.active>a, .fav_airports_tabs .nav-tabs>li.active>a:focus, .fav_airports_tabs .nav-tabs>li.active>a:hover{ color:#fff; }
  .map_align_mobile{
    width: 215px;
    height: 215px;
...
<div id="page">
  ...
  <div class="row">
    ...
    <div class="col-md-9 p-l-0">
        <p style="text-align: justify;margin-bottom: 15px;">India's fast growing economy has created the need for Business Aircraft to travel to unfamiliar airports which creates an added set of pressures for a Pilot. Our Airport information gives all the tools needed to take the worry out of trip planning to anywhere in India.</p>
    </div>
    <div class="col-md-3 p-lr-0">
            <form class="ar_search " target="_blank" id="form" action="{{action('AirportController@search') }}" method="POST">
                        <div class="input-group">
                           {{ csrf_field() }}
                            <input type="text" autocomplete="off" class="form-control alphabets form-group ar_search_input1 ui-autocomplete-input auto_caps" placeholder="Airport Name or ICAO Code" name="search" id="Info" required="" autocomplete="off" style="border-radius: 2px" maxlength="3">
                            <div class="input-group-btn">
                                <button class="btn newbtnv1 ar_search_btn1" type="submit"><i class="glyphicon glyphicon-search"></i></button>
                            </div>
                        </div>
                        <span style="color:red;"></span>
            </form>
    </div>
            <div class="col-md-12 text-center fav_airports_tabs">
                <div class="col-md-offset-2 col-md-8 but-sec">
                <ul class="nav nav-tabs">
                  @foreach($ap as $ap_key=>$airports)
                    @if($ap_key==0)
                     <li class="active"><a href="#{{ $airports->airport_code}}" data-toggle="tab" class="btn newbtnv1 but_wd_index airport_tab">{{ $airports->airport_code}}</a></li>
                    @else
                      <li><a href="#{{ $airports->airport_code}}" data-toggle="tab" class="airport_tab btn newbtn_blackv1 but_wd_index">{{ $airports->airport_code}}</a></li>
                    @endif  
                  @endforeach
                 </ul>
                </div>
  ...
  <!-- sumit start !-->
    <div class="container cust-container">
      ...
      @foreach($ap as $ap_key=>$airports)
       <div  @if($ap_key==0) class="tab-pane active" @else class="tab-pane" @endif id="{{$airports->airport_code}}">
        <div class="row">
            <div class="col-md-12 p-lr-0">
                <p class="airport_heading auto_caps">{{substr($airports->airport_name,7)}}</p>
            </div>
            <div class="col-md-12">
                <div class="but-sec">
                    <button type="button" class="btn newbtnv1 but_wd" data-toggle="modal" data-target="#metar_modal" >METAR &amp; TAF</button>
                    <a href='{{url("notams/airport/$airports->airport_code")}}' target="_blank" class="btn newbtnv1 but_wd"> NOTAMS (0)</a>
                ...
                <div class="col-md-9">
                                ...
                                <table class="table">
                                  ...
                                  @foreach($airports->runway as $runways)
                                          @if(isset($runways->dimension))
                                           <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->xrunway."/".$runways->yrunway}} Dimension :</b></div>
                                           <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->dimension}}</div>
                                          @else
                                  ...
                                  @foreach($airports->runway as $runways)        
                                          @if(isset($runways->xcoordinates))
                                              <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->xrunway}} Coordinates :</b></div>
                                              <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->xcoordinates}}</div>
                                          @endif
                                  @endforeach        
                                  @foreach($airports->runway as $runways)            
                                          @if(isset($runways->ycoordinates))
                                              <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->yrunway}} Coordinates :</b></div>
                                              <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->ycoordinates}}</div>
                                          @endif
                                  @endforeach        
                                  @foreach($airports->runway as $runways)             
                                          @if(isset($runways->xelevation))
                                              <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->xrunway}} Elevation :</b></div>
                                              <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->xelevation}}</div>
                                          @endif
                                 @endforeach        
                                  @foreach($airports->runway as $runways)           
                                          @if(isset($runways->yelevation))  
                                              <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->yrunway}} Elevation :</b></div>  
                                              <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->yelevation}}</div>
                                          @endif
                                   @endforeach        
                                  @foreach($airports->runway as $runways)             
                                          @if(isset($runways->xdisplaced_threshold))
                                              <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->xrunway}} Displaced Threshold :</b></div>
                                              <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->xdisplaced_threshold}}</div>
                                          @endif
                                   @endforeach        
                                  @foreach($airports->runway as $runways)           
                                          @if(isset($runways->ydisplaced_threshold))
                                              <div class="col-md-4 col-xs-12 runway_align"><b>Runway {{ $runways->yrunway}} Displaced Threshold :</b></div>
                                              <div class="col-md-2 col-xs-12 runway_align p-lr-0">{{ $runways->ydisplaced_threshold}}</div>
                                         @endif 
                                    @endforeach 
                                    </tbody>
                                </table>
                    ...
                    <div class="col-md-12 ac_sec">
                        ...
                        <div class="ac_inner_sec">
                            ...
                            <div class="col-sm-4 text-right airport_communications_align airport_communications_align_tablet" style="font-weight: bold;">{{ substr($communications->comm_details,0,$str+1)}}</div>
                            <div class="col-sm-2 text-left airport_communications_align p-lr-0">{{ substr($communications->comm_details,$str+1)}}</div>
                            @endforeach
                        </div>
                    </div>
                    @endif
                                ...
                                <table class="table m-b-0">
                                    ...
                                    <tbody>
                                        @foreach($airports->near_by_airport as $near_by_airport)
                                          <tr>
                                           <td class="aiport_heading_distance">{{ $near_by_airport->nearbyairportid.'-'.$near_by_airport->name}}</td>
                                           <td class="aiport_heading_distance">{{ $near_by_airport->heading}}</td>
                                           <td class="aiport_heading_distance">{{ $near_by_airport->distance}}</td>
                                          </tr> 
                                         @endforeach
                                    </tbody>
                                </table>
                ...
                <div class="col-md-3 p-lr-0">
                    ...
                    <div class="satellite_va">
                        ...
                        <div class="col-md-12 p-tb-15">
                             <a target="_blank" href="http://maps.google.com/maps?ll={{$airports->lat}},{{$airports->long}}&amp;t=h&amp;z=16"><img class="map_align_mobile" src="https://maps.google.com/maps/api/staticmap?center={{$airports->lat}},{{$airports->long}}&amp;size=300x300&amp;zoom=14&amp;maptype=satellite&amp;sensor=false&key=AIzaSyD8kpfNY_zchycTS1zvLHMgxE3W1N06EkY" alt="Google Maps Satellite View" class="tp_main_img"></a>
                        </div>
                    </div>
...
<script>
  $(".airport_tab").click(function(){ 
   // $('.nav-tabs li.active').removeClass('active');
   $('.airport_tab').removeClass('newbtnv1').addClass('newbtn_blackv1');
    $(this).removeClass('newbtn_blackv1') 
    $(this).addClass('newbtnv1') });
  ...
  $( function() {
     ...
     $.ajax({
        url: "{{action('AirportController@autosuggest_post') }}",
        dataType:"json",  
     ...
     $(".ar_search_btn1").click(function(){
      ...
      $.ajax({
       ...
       if(res==false)
       {
         $('.ar_search_input1').css('border-color','red');
         return false;
        }
    });
  });
